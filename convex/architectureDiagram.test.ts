...
import { generateArchitectureDiagram, type DiagramSnapshot, type DiagramSnapshotFile } from "./lib/architectureDiagram";
...
function file(path: string, overrides: Partial<DiagramSnapshotFile> = {}): DiagramSnapshotFile {
...
describe("ArchitectureDiagramGenerator", () => {
  ...
  test("handles a repository with no recognised top-level directories", () => {
    const snapshot: DiagramSnapshot = {
      ...
      files: [file("README.md", { isImportant: true }), file("package.json", { isConfig: true, isImportant: true })],
    };
  ...
  test("reuses node ids deterministically when the same path appears at multiple depths", () => {
    const snapshot: DiagramSnapshot = {
      ...
      files: [...FIXTURE_SNAPSHOT.files, dir("src-extras"), file("src-extras/notes.md", { isImportant: true })],
    };
