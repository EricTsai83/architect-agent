...
import { getAllowedReturnToOriginsFromEnv, normalizeReturnToUrl } from "./returnTo";
...
describe("returnTo allowlist", () => {
  test("returns sanitized full returnTo URL when allowlisted", () => {
    ...
    const result = normalizeReturnToUrl("https://app.systify.dev/settings?tab=integrations", allowlist);
  ...
  test("rejects non-allowlisted origin", () => {
    ...
    expect(() => normalizeReturnToUrl("https://evil.example.com/phish", allowlist)).toThrow(/not allowlisted/i);
  });
  ...
  test("accepts localhost http redirect origin when allowlisted", () => {
    ...
    expect(normalizeReturnToUrl("http://localhost:5173/callback?source=github", allowlist)).toBe(
      "http://localhost:5173/callback?source=github",
    );
  });
  ...
  test("throws when returnTo is malformed or not absolute", () => {
    ...
    expect(() => normalizeReturnToUrl("/relative/path", allowlist)).toThrow("returnTo must be an absolute URL.");
    expect(() => normalizeReturnToUrl("not-a-url", allowlist)).toThrow("returnTo must be an absolute URL.");
  });
  ...
  test("throws when returnTo includes credentials", () => {
    ...
    expect(() =>
      normalizeReturnToUrl("https://user:pass@app.systify.dev/settings?tab=integrations", allowlist),
  ...
  test("rejects localhost http redirect when localhost origin is not allowlisted", () => {
    ...
    expect(() => normalizeReturnToUrl("http://localhost:5173/callback?source=github", allowlist)).toThrow(
      /not allowlisted/i,
    );
  });

  test("parses allowlist from env and canonicalizes origins", () => {
    process.env[RETURN_TO_ALLOWLIST_ENV] = " https://app.systify.dev/ , https://preview.systify.dev ";
  ...
  test("rejects non-localhost http origin entries in env allowlist", () => {
    ...
    expect(() => getAllowedReturnToOriginsFromEnv()).toThrow(/only allowed for localhost development origins/i);
  });

  test("throws when env allowlist is missing or empty", () => {
    delete process.env[RETURN_TO_ALLOWLIST_ENV];
    expect(() => getAllowedReturnToOriginsFromEnv()).toThrow(`${RETURN_TO_ALLOWLIST_ENV} is required`);
