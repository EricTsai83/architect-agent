...
### 4. Repository file count 會在大型 repo 低估
- Severity: Medium
...
- Evidence:
  - 前端只有警告文字，送出按鈕沒有因 `deepModeAvailable === false` 停用
  - `handleSendMessage()` 也沒擋
  - 後端深度分析本身會失敗，但 UX 已經太晚
- Why it matters:
  - 使用者明知 sandbox 已失效仍可送出 deep request
  - 產生多餘 job、錯誤訊息與操作混亂
- Fix direction:
  - UI 禁止送出 deep request
  - submit handler 再做一次 guard

### 7. 非同步操作失敗時多數沒有使用者可見回饋
- Severity: Medium
- Files:
...
- Evidence:
  - `useAsyncCallback()` 只有 `try/finally`，沒有集中錯誤處理
...
### 8. Auth loading 與 runtime error 暴露體驗偏弱
- Severity: Medium
- Files:
  - `src/App.tsx`
